...
class PluginRTCVideoCaptureController : NSObject {
	...
	*/
	...
	func startCapture() -> Bool {
		...
		PluginUtils.debug("PluginRTCVideoCaptureController#startCapture Capture started, device:%@, format:%@", device!, deviceFormat!);
	...
	func stopCapture() {
		...
		if (isCapturing) {
			...
			PluginUtils.debug("PluginRTCVideoCaptureController#stopCapture Capture stopped");
		}
	}
	...
	fileprivate func findDevice() -> AVCaptureDevice? {
		...
		if (facingMode.count > 0) {
			...
			PluginUtils.debug("PluginRTCVideoCaptureController#findDevice facingMode:%@ env:%@ user:%@", facingMode,
		...
		PluginUtils.debug("PluginRTCVideoCaptureController#findDevice device:%@", device!);
		...
		if (minFrameRate > 0 || maxFrameRate > 0) {
			...
			PluginUtils.debug("PluginRTCVideoCaptureController#findDevice deviceFrameRate:%i", deviceFrameRate!);
	...
	*/
	...
	fileprivate func findFormatForDevice(device: AVCaptureDevice) -> AVCaptureDevice.Format? {
		...
		if (minAspectRatio > 0 || maxAspectRatio  > 0) {
			...
			PluginUtils.debug("PluginRTCVideoCaptureController#findFormatForDevice contraints using aspectRatioRange - width:%i/%i, height:%i/%i, aspectRatio: %f/%f", minWidth, maxWidth, minHeight, maxHeight, minAspectRatio, maxAspectRatio);
		...
		// If no aspectRatioRange and no dimension is provided use default dimension
			...
			PluginUtils.debug("PluginRTCVideoCaptureController#findFormatForDevice contraints using default - width:%i/%i, height:%i/%i, aspectRatio: %f/%f", minWidth, maxWidth, minHeight, maxHeight, minAspectRatio, maxAspectRatio);
		}
		...
		for format: Any in formats {
			...
			PluginUtils.debug("PluginRTCVideoCaptureController#findFormatForDevice device format - width:%i, height:%i, aspectRatio: %f, frameRateRanges:%f/%f", dimension.width, dimension.height, aspectRatio, frameRates.minFrameRate, frameRates.maxFrameRate);
