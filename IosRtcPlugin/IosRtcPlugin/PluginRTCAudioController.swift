...
class PluginRTCAudioController {
	...
	static func setCategory() -> Void {
		// Enable speaker
		PluginUtils.debug("PluginRTCAudioController#setCategory()")
		...
		if currentRoute.outputs.count != 0 {
			for description in currentRoute.outputs {
				if (
					...
					PluginUtils.debug("PluginRTCAudioController#setOutputSpeakerIfNeed() | external audio output plugged in -> do nothing")
				} else {
					PluginUtils.debug("PluginRTCAudioController#setOutputSpeakerIfNeed() | external audio pulled out")

					if (speakerEnabled) {
						do {
							try audioSession.overrideOutputAudioPort(AVAudioSession.PortOverride.speaker)
						} catch {
							PluginUtils.debug("PluginRTCAudioController#setOutputSpeakerIfNeed() | ERROR \(error)")
						};
					}
				}
			}
	...
	static func selectAudioOutputSpeaker() {
		// Enable speaker
		PluginUtils.debug("PluginRTCAudioController#selectAudioOutputSpeaker()")
	...
	static func selectAudioOutputEarpiece() {
		...
		PluginUtils.debug("PluginRTCAudioController#selectAudioOutputEarpiece()")
	...
	@objc dynamic fileprivate func audioRouteChangeListener(_ notification:Notification) {
		...
		case AVAudioSession.RouteChangeReason.newDeviceAvailable.rawValue:
			PluginUtils.debug("PluginRTCAudioController#audioRouteChangeListener() | headphone plugged in")
		case AVAudioSession.RouteChangeReason.oldDeviceUnavailable.rawValue:
			PluginUtils.debug("PluginRTCAudioController#audioRouteChangeListener() | headphone pulled out -> restore state speakerEnabled: %@", PluginRTCAudioController.speakerEnabled ? "true" : "false")
