...
class PluginRTCPeerConnectionConfig {
	...
	init(pcConfig: NSDictionary?) {
		PluginUtils.debug("PluginRTCPeerConnectionConfig#init()")
		...
		if iceServers != nil {
			for iceServer: NSDictionary in iceServers! {
				...
				if (urls != nil && urls!.count > 0) {
					PluginUtils.debug("PluginRTCPeerConnectionConfig#init() | adding ICE server [url:'%@', username:'%@', password:'******']",
	...
	deinit {
		PluginUtils.debug("PluginRTCPeerConnectionConfig#deinit()")
	}
	
	func getConfiguration() -> RTCConfiguration {
		PluginUtils.debug("PluginRTCPeerConnectionConfig#getConfiguration()")
