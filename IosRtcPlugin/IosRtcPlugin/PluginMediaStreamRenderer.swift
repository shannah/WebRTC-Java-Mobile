...
class PluginMediaStreamRenderer : NSObject, RTCEAGLVideoViewDelegate {
	...
	init(
		...
		PluginUtils.debug("PluginMediaStreamRenderer#init()")
        ...
        PluginUtils.debug("PluginMediaStreamRenderer#init() | creating Video View");
        if (!Thread.isMainThread) {
                PluginUtils.debug("WARNING: PluginMediaStreamRenderer#init() | creating Video View off main thread");
        }
		self.videoView = RTCEAGLVideoView()
        self.videoView.setSize(CGSize(width:0, height:0))
	...
	deinit {
		PluginUtils.debug("PluginMediaStreamRenderer#deinit()")
	}

	func run() {
		PluginUtils.debug("PluginMediaStreamRenderer#run()")

		self.videoView.delegate = self
	}

	func render(_ pluginMediaStream: PluginMediaStream) {
		PluginUtils.debug("PluginMediaStreamRenderer#render()")
		...
		if self.rtcVideoTrack != nil {
            PluginUtils.debug("PluginMediaStreamRenderer#render() > rtcVideoTrack.add(videoView)")
            self.videoView.setSize(CGSize(width:0, height:0))
			self.rtcVideoTrack!.add(self.videoView)
			pluginVideoTrack?.registerRender(render: self)
            PluginUtils.debug("PluginMediaStreamRenderer#render() > videoSize:")
        } else {
            PluginUtils.debug("PluginMediaStreamRenderer#render() > no video track found")
        }
	}

	func mediaStreamChanged() {
		PluginUtils.debug("PluginMediaStreamRenderer#mediaStreamChanged()")
		...
		if oldRtcVideoTrack != nil && self.rtcVideoTrack != nil &&
			...
			PluginUtils.debug("PluginMediaStreamRenderer#mediaStreamChanged() | same video track as before")
		}

		// Different video track.
		else if oldRtcVideoTrack != nil && self.rtcVideoTrack != nil &&
			oldRtcVideoTrack!.trackId != self.rtcVideoTrack!.trackId {
			PluginUtils.debug("PluginMediaStreamRenderer#mediaStreamChanged() | has a new video track")
		...
		// Did not have video but now it has.
			...
			PluginUtils.debug("PluginMediaStreamRenderer#mediaStreamChanged() | video track added")
		...
		// Had video but now it has not.
			...
			PluginUtils.debug("PluginMediaStreamRenderer#mediaStreamChanged() | video track removed")
	...
	func refresh(_ data: NSDictionary) {
		...
		PluginUtils.debug("PluginMediaStreamRenderer#refresh() [elementLeft:%@, elementTop:%@, elementWidth:%@, elementHeight:%@, videoViewWidth:%@, videoViewHeight:%@, visible:%@, opacity:%@, zIndex:%@, mirrored:%@, clip:%@, borderRadius:%@]",
	...
	func save() -> String {
		PluginUtils.debug("PluginMediaStreamRenderer#save()")
	...
	func stop() {
		PluginUtils.debug("PluginMediaStreamRenderer | video stop")
	...
	func close() {
		PluginUtils.debug("PluginMediaStreamRenderer#close()")
		self.closed = true
		self.reset()
	...
	fileprivate func reset() {
		PluginUtils.debug("PluginMediaStreamRenderer#reset()")
	...
	func videoView(_ videoView: RTCVideoRenderer, didChangeVideoSize size: CGSize) {
		...
		PluginUtils.debug("PluginMediaStreamRenderer | video size changed [width:%@, height:%@]",
